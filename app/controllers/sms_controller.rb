class SmsController < ApplicationController
  ...
  private
    ...
    def sm_params
      params.require(:sm).permit(:title, :message, :to)
    end

    def generate_url(sm, emp)
      ENV['SMS_PROVIDER_URL'] + '?user=' + ENV['SMS_USERNAME'] + '&pwd=' + ENV['SMS_PASSWORD'] + '&sender=' + ENV['SMS_SENDER'] + '&reciever=' + emp.number + '&msg-data=' + URI.encode(sm.message) + '&response=' + 'string'
    end
end
